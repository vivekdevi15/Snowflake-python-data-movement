...
cur.execute("""
CREATE OR REPLACE STAGE PATIENT360_DB.TEST.IMMUNIZATIONS_STAGING
file_format = PATIENT360_DB.TEST.FF_P360
""")

cur.execute("PUT file://my_data/immunizations.csv @PATIENT360_DB.TEST.IMMUNIZATIONS_STAGING")

#Moving data from staging to source table with all correct data types

cur.execute("""
COPY INTO PATIENT360_DB.TEST.IMMUNIZATIONS
(
DATE,
PATIENT,
ENCOUNTER,
...
FROM
( SELECT  $1, $2, $3, $4, $5, $6
FROM @PATIENT360_DB.TEST.IMMUNIZATIONS_STAGING
)on_error = 'Continue';
""")

print("Immunizations data moved to table")
