...
cur.execute("""
CREATE OR REPLACE STAGE PATIENT360_DB.TEST.MEDICATIONS_STAGING
file_format = PATIENT360_DB.TEST.FF_P360
""")

cur.execute("PUT file://my_data/medications.csv @PATIENT360_DB.TEST.MEDICATIONS_STAGING")

#Moving data from staging to source table with all correct data types

cur.execute("""
COPY INTO PATIENT360_DB.TEST.MEDICATIONS
(
START_DATE,
STOP_DATE,
PATIENT,
...
FROM @PATIENT360_DB.TEST.MEDICATIONS_STAGING
)on_error = 'Continue';
""")

print("Medications data moved to table")
