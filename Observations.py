...
cur.execute("""
CREATE OR REPLACE STAGE PATIENT360_DB.TEST.OBSERVATIONS_STAGING
file_format = PATIENT360_DB.TEST.FF_P360
""")
cur.execute("PUT file://my_data/observations.csv @PATIENT360_DB.TEST.OBSERVATIONS_STAGING")

#Moving data from staging to source table with all correct data types

cur.execute("""
COPY INTO PATIENT360_DB.TEST.OBSERVATIONS
(
DATE,
PATIENT,
ENCOUNTER,
...
FROM @PATIENT360_DB.TEST.OBSERVATIONS_STAGING
)on_error = 'Continue';
""")

print("Observations data moved to table")
