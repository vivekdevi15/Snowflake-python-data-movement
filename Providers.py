...
cur.execute("""
CREATE OR REPLACE STAGE PATIENT360_DB.TEST.PROVIDERS_STAGING
file_format = PATIENT360_DB.TEST.FF_P360
""")
cur.execute("PUT file://my_data/providers.csv @PATIENT360_DB.TEST.PROVIDERS_STAGING")

#Moving data from staging to source table with all correct data types

cur.execute("""
COPY INTO PATIENT360_DB.TEST.PROVIDERS
(
ID,
ORGANIZATION,
NAME,
...
FROM @PATIENT360_DB.TEST.PROVIDERS_STAGING
)on_error = 'Continue';
""")
    
print("Providers data moved to table")
