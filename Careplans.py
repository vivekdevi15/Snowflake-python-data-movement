...
cur.execute("""
CREATE OR REPLACE STAGE PATIENT360_DB.TEST.CAREPLANS_STAGING
file_format = PATIENT360_DB.TEST.FF_P360
""")

cur.execute("PUT file://my_data/careplans.csv @PATIENT360_DB.TEST.CAREPLANS_STAGING")

#Moving data from staging to source table with all correct data types

cur.execute("""
COPY INTO PATIENT360_DB.TEST.CAREPLANS
(
ID,
START_DATE,
STOP_DATE,
...
FROM @PATIENT360_DB.TEST.CAREPLANS_STAGING
)on_error = 'Continue';
""")

print("Careplans data moved to table")
